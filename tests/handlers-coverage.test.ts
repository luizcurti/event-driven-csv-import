...
describe('handler coverage', () => {
  it('supports upload success and validation failures', async () => {
    ...
    await expect(
      handler(event('not-json', { 'content-type': 'application/json' })),
    ).rejects.toThrow(
    ...
    await expect(
      handler(event('', { 'content-type': 'multipart/form-data' })),
    ).rejects.toThrow('Multipart boundary is missing.');
    await expect(
      handler(
    ...
    const defaultMultipartHandler = createUploadHandler(
      defaultMultipartDependencies,
    );
    ...
    expect(
      (await defaultMultipartDependencies.store.listImports())[0],
    ).toMatchObject({ filename: 'import.csv' });
  });
  ...
  it('splits imports and handles missing inputs', async () => {
    ...
    expect(await dependencies.store.getImport(importRecord.id)).toMatchObject({
      status: 'QUEUED',
      totalChunks: 2,
      totalRecords: 2,
    });
    ...
    const fallbackImport = await seedImport(fallbackDependencies, {
      chunkSize: undefined as unknown as number,
    });
    ...
    expect(
      (await createSplitHandler(fallbackDependencies)(fallbackImport.id))
        .totalChunks,
    ).toBe(1);
    ...
    expect(
      (await createSplitHandler(emptyDependencies)(emptyImport.id)).totalChunks,
    ).toBe(0);
    await expect(
      createSplitHandler(createTestDependencies())('missing'),
    ...
    await expect(
      createSplitHandler(missingObjectDependencies)('import-1'),
    ).rejects.toThrow('Source CSV not found in storage.');
  });

  it('processes worker chunks across all result branches', async () => {
    const completedDependencies = createTestDependencies();
    const completedImport = await seedImport(completedDependencies, {
      status: 'QUEUED',
    });
    ...
    const firstAttempt = await createWorkerHandler(completedDependencies)(
      chunkMessage,
    );
    ...
    const redeliveredAttempt = await createWorkerHandler(completedDependencies)(
      chunkMessage,
    );
    expect(redeliveredAttempt.processedChunks).toBe(1);

    const partialDependencies = createTestDependencies();
    const partialImport = await seedImport(partialDependencies, {
      status: 'QUEUED',
    });
    ...
    const failedImport = await seedImport(failedDependencies, {
      status: 'QUEUED',
    });
  ...
  it('aggregates results and reports statuses', async () => {
    ...
    expect(
      (await createAggregatorHandler(completedDependencies)(completedImport.id))
        .status,
    ).toBe('COMPLETED');
    ...
    expect(
      (await createAggregatorHandler(partialDependencies)(partialImport.id))
        .status,
    ).toBe('PARTIAL_SUCCESS');
    ...
    expect(
      (await createAggregatorHandler(failedDependencies)(failedImport.id))
        .status,
    ).toBe('FAILED');
    await expect(
      createAggregatorHandler(createTestDependencies())('missing'),
    ).rejects.toThrow('Import not found.');
    ...
    const pendingImport = await seedImport(pendingDependencies, {
      totalChunks: 2,
      status: 'RUNNING',
    });
    ...
    const pendingResult = await createAggregatorHandler(pendingDependencies)(
      pendingImport.id,
    );
    expect(pendingResult.status).toBe('RUNNING');
    expect(pendingResult.processedChunks).toBe(1);
    expect(
      await pendingDependencies.store.getImport(pendingImport.id),
    ...
    expect(JSON.parse((await statusHandler()).body ?? '{}')).toMatchObject({
      items: [expect.objectContaining({ id: statusImport.id })],
    });
    expect(
      JSON.parse((await statusHandler(statusImport.id)).body ?? '{}'),
    ).toMatchObject({ id: statusImport.id, status: 'UPLOADED' });
